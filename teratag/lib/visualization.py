...
# カラーコード方式で可視化(厚み、横の数、縦の数) TDS反射タグ用
def colorcode_new(best_pred, x, y, concentration_color_type, name):
    plt.style.use('default')
    im = Image.new('RGB', (100 + x * 108, 100 + y * 100), (128, 128, 128))
    draw = ImageDraw.Draw(im)
    position_x = []
    position_y = []
    first_position_y = 30
    ...
    for i, j in enumerate(best_pred):
        ...
        for k in range(y):
            if int(i) < int(x) * int(k+1):
                draw.ellipse((position_x[k], position_y[k], position_x[k] + 100, position_y[k+1] - 10),
                                fill=(concentration_color_type[j][0], concentration_color_type[j][1], concentration_color_type[j][2]))
                position_x[k] += 110
